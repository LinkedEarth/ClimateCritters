...
To use any stochastic solver, define `sde_noise` on your model subclass. It receives the current time and state and returns a diffusion vector with the same shape as the integrated state:
...
If `sde_noise` is absent or not callable, the diffusion term is zero and the stochastic integrators reduce to their deterministic equivalents. See [Noise](noise.qmd) for more on the distinction between process and observation noise.
