...
| `cc.core.Forcing` | `sigma=cc.core.Forcing(...)` | Evaluated via `get_forcing(t)` |
...
`Forcing` wraps any time-dependent input signal and provides a uniform `get_forcing(t)` interface to the model. It accepts:

**A callable:**

```python
forcing = cc.core.Forcing(lambda t: 1360.0 + 5.0 * np.sin(2 * np.pi * t / 11.0))
```

**An array with a time axis:**

```python
forcing = cc.core.Forcing(data=my_array, time=my_time, interpolation='cubic')
```

**A CSV dataset** (built-in datasets include TSI reconstructions and insolation):

```python
forcing = cc.core.Forcing.from_csv(dataset='vieira_tsi')
```

### Composable sequences

For scenarios with distinct phases (e.g. a spin-up, a ramp, and a perturbation), use `Forcing.from_sequence()` with `Hold`, `Ramp`, and `Harmonic` elements:

```python
forcing = cc.core.Forcing.from_sequence([
    cc.core.Hold(duration=100, value=1360.0),
    cc.core.Ramp(duration=50, y0=1360.0, yf=1380.0, shape='linear'),
    cc.core.Hold(duration=100, value=1380.0),
])
```
...
forcing = cc.core.Forcing.from_sequence([
    cc.core.Hold(duration=500, value=1360.0),
    cc.core.Ramp(duration=200, y0=1360.0, yf=1390.0, shape='linear'),
])
