...
```python
cc.Hold(duration=100, value=280.0)   # 100 time units at 280
cc.Hold(dt=50,        value=1360.0)  # dt is an alias for duration
cc.Hold(tf=200,       value=0.0)     # ends at absolute time 200
```
...
cc.Ramp(duration=100, y0=280.0, yf=560.0, shape='linear')

# Cosine (S-curve) ramp; y0 inherits from the previous segment if omitted
cc.Ramp(duration=100, yf=560.0, shape='cosine')

# Specify amplitude instead of endpoint: yf = y0 + A
cc.Ramp(duration=100, A=280.0)
```
...
```python
cc.Harmonic(duration=20, period=1.0, A=0.5, center=0.0)
```
...
```python
cc.ForcingElement(lambda t: np.exp(-0.01 * t), duration=50.0)
```
...
```python
scenario = scenario + cc.Ramp(duration=100, yf=280.0)
...
f = cc.Forcing.from_sequence([
    ...
    cc.Ramp(duration=50, y0=0.0, yf=1.0),
], label='hosing')
```
...
seq  = cc.Hold(10, value=0.0) + elem + cc.Hold(10, value=0.0)
f    = seq.compile()
